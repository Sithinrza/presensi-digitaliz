...
| FILE: edit.blade.php
|--------------------------------------------------------------------------
| Asumsi: Variabel $penetapan (Model JadwalKaryawan), $karyawans, $jadwalKerjas selalu tersedia
--}}

@php
$isEdit = true; // Selalu true di file edit.blade
$formTitle = 'Edit Penetapan Jadwal Karyawan';
$formAction = route('admin.penetapan.update', $penetapan->id_karyawan); // Menggunakan id_karyawan sesuai model Anda
$karyawanIdSaatIni = $penetapan->id_karyawan;
$jadwalIdSaatIni = $penetapan->id_jadwal_kerja;
@endphp
...
<header class="bg-indigo-950 p-6 pt-10 pb-28 rounded-b-3xl shadow-xl relative z-10 -mx-4 md:mx-0">
    <div class="flex items-center space-x-4 text-white max-w-4xl mx-auto md:mx-0">
        ...
        <div>
            ...
            <p class="text-indigo-200 text-sm mt-0.5 font-medium">Atur ulang template jadwal karyawan.</p>
        </div>
    </div>
</header>
...
<main class="px-4 -mt-20 relative z-20 max-w-4xl mx-auto">
    ...
    <form action="{{ $formAction }}" method="POST" class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
        @csrf
        {{-- Di mode EDIT, method harus PUT/PATCH --}}
        @method('PUT')
        ...
        <div class="p-6 md:p-8 space-y-8">
            ...
            <section>
                ...
                <div class="relative">
                    ...
                    {{-- Di mode edit, select box dinonaktifkan (disabled) --}}
                    <select name="id_karyawan_disabled" id="id_karyawan" disabled
                        class="w-full pl-11 pr-10 py-4 rounded-2xl border-gray-200 bg-gray-100 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none outline-none text-sm font-bold text-gray-700 cursor-not-allowed shadow-sm">
                ...
                {{-- Input Hidden untuk mengirim ID Karyawan yang sebenarnya ke Controller --}}
                <input type="hidden" name="id_karyawan" value="{{ $penetapan->id_karyawan }}">
                <p class="mt-2 text-xs text-gray-500">Nama karyawan tidak dapat diubah saat mode edit relasi.</p>
            </section>
            ...
            <section>
                ...
                <div class="relative">
                    ...
                    <select name="id_jadwal_kerja" id="id_jadwal_kerja"
                        ...
                        <option value="" disabled>-- Pilih Template Jadwal --</option>
...
@push('scripts')
{{-- JavaScript tetap sama dan berfungsi dengan baik --}}
<script>
...
function renderJadwalDetail(jadwalId) {
    ...
    sortedDetails.forEach(detail => {
        ...
        // Hati-hati: Array hariIndonesia hanya sampai Sabtu (index 5). Index 0 = Senin, Index 5 = Sabtu.
        // Jika Anda ingin mendeteksi Minggu, array hariIndonesia harus ditambahkan 'Minggu'.
        // Saya asumsikan Anda hanya menampilkan Senin-Sabtu.
        const isWeekend = hariIndex === 5; // Cuma Sabtu
...
// Panggil saat halaman pertama kali dimuat (WAJIB untuk mode EDIT)
