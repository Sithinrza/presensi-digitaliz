        ...
        <div class="w-full max-w-md bg-white rounded-2xl shadow-lg overflow-hidden">
            <header class="bg-indigo-950 text-white p-4 flex items-center justify-between">
                ...
                <button type="button" class="p-1 rounded-full hover:bg-white/20">
                    {{-- TOMBOL KEMBALI: Ganti 'datapegawai.html' dengan route index --}}
            ...
            <main class="p-6">
                <div class="flex flex-col items-center text-center mb-6">
                    ...
                    <div class="flex items-center space-x-1">
                        ...
                        {{-- Status aktif (Jika ada kolom di DB) --}}
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <span class="text-gray-800 col-span-2">: {{ $karyawan->user?->email ?? '-' }}</span> {{-- Asumsi relasi user ada --}}
                    </div>
                    <div class="grid grid-cols-3 gap-2">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <span class="text-gray-800 col-span-2">: {{ $karyawan->agama?->name ?? '-' }}</span> {{-- Asumsi relasi agama ada --}}
                    </div>
                    <div class="grid grid-cols-3 gap-2">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <span class="text-gray-800 col-span-2">: {{ $karyawan->pendidikanTerakhir?->name ?? '-' }}</span> {{-- Asumsi relasi pendidikanTerakhir ada --}}
                    </div>
                    <div class="grid grid-cols-3 gap-2">
                ...
                <div class="mt-8 flex items-center justify-center space-x-4">
                    ...
                    {{-- TOMBOL DELETE (Harus pakai form dan method DELETE) --}}
                    <form action="{{ route('admin.karyawan.destroy', $karyawan->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus {{ $karyawan->nama_lengkap }}?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="flex items-center space-x-2 w-full justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:ring-red-300">
                            <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
