            ...
            <div class="bg-white p-5 rounded-xl shadow-md">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="tempat_lahir" id="tempat_lahir" value="{{ old('tempat_lahir', $karyawan->tempat_lahir) ?? '' }}" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly>
                    </div>
                    <div class="relative">
                         <label for="tanggal-lahir" class="block mb-1 text-sm font-medium text-gray-500">Tanggal Lahir</label>
                         <input type="text" name="tanggal_lahir" value="{{ old('tanggal_lahir', $karyawan->tanggal_lahir ? \Carbon\Carbon::parse($karyawan->tanggal_lahir)->format('d M Y') : '') }}" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly>
                    ...
                    <div>
                        ...
                        <input type="text" name="jenis_kelamin" value="{{ old('jenis_kelamin', $karyawan->jenis_kelamin) }}" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="tel" id="no_telepon" name="no_telepon" value="{{ old('no_telepon', $karyawan->no_telepon) }}" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="08xxxxxxxxxx">
                    </div>
                </div>
            </div>
            <div class="bg-white p-5 rounded-xl shadow-md">
                <h2 class="text-lg font-bold text-gray-800 mb-4">Akun Pegawai</h2>
                 <div class="space-y-4">
                    <div>
                        ...
                        <input type="email" id="username" name="email" value="{{ old('email', $karyawan->user?->email) }}" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly>
                    </div>
                    <div>
                        <label for="password" class="block mb-1 text-sm font-medium text-gray-500">Password</label>
                        <input type="password" id="password" name="password" value="{{ old('password', $karyawan->user?->password) }}" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly>
                    </div>
                    <div>
            ...
            <div class="bg-white p-5 rounded-xl shadow-md">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="status-karyawan" name="status_karyawan" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            @php
                                $currentStatus = old('status_karyawan', $karyawan->status_karyawan);
                            @endphp
                            <option value="Aktif" {{ $currentStatus == 'Aktif' ? 'selected' : '' }}>
                                Aktif
                            </option>

                            <option value="Tidak Aktif" {{ $currentStatus == 'Tidak Aktif' ? 'selected' : '' }}>
                                Tidak Aktif
                            </option>
                    ...
                    <div>
                        ...
                        <select id="jabatan" name="jabatan_id" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                           @php $jid = old('jabatan_id', $karyawan->jabatan_id); @endphp
                            @foreach($jabatans as $jabatan)
                                <option value="{{ $jabatan->id }}"
                                    @if($jid == $jabatan->id) selected @endif >
                                    {{ $jabatan->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="divisi" name="divisi_id" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                             @php $did = old('divisi_id', $karyawan->divisi_id); @endphp
                            @foreach($divisis as $divisi)
                                <option value="{{ $divisi->id }}"
                                    @if($did == $divisi->id) selected @endif >
                                    {{ $divisi->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="posisi" name="posisi_id" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                              @php $pid = old('posisi_id', $karyawan->posisi_id); @endphp
                            @foreach($posisis as $posisi)
                                <option value="{{ $posisi->id }}"
                                    @if($pid == $posisi->id) selected @endif >
                                    {{ $posisi->name }}
                                </option>
                            @endforeach
                        </select>
