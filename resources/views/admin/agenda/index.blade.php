<x-admin-layout>
    ...
    <div class="relative min-h-screen pb-24">
        ...
        <!-- Konten Utama (Filter dan Daftar) -->
        <main class="p-4 pt-6 relative z-20 space-y-6">

            <!-- Filter Tanggal dan Tombol Tambah -->
            <section class="bg-white pt-5 rounded-xl shadow-lg">
                <!-- Tombol Tambah (Di sini diletakkan agar di kanan atas) -->
                <div class="px-5 mb-4 flex justify-end">
                    <a href="{{ route('admin.agenda.create') }}" class="inline-flex flex-shrink-0 text-white bg-indigo-800 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-4 py-2 text-center shadow-lg items-center justify-center space-x-2">
                ...
                <form action="{{ route('admin.agenda.index') }}" method="GET" id="filter-form" class="space-y-4">
                    <div class="px-5 pb-5">
                        ...
                        <div class="flex items-end space-x-3">
                            ...
                            <!-- Tombol Lihat Semua Agenda (Di samping filter tanggal) -->
                            <a href="{{ route('admin.agenda.index', ['tanggal' => 'all']) }}" class="flex-shrink-0 whitespace-nowrap text-sm font-medium rounded-lg px-4 py-2.5 h-full
                        ...
                        <div class="bg-white p-4 rounded-xl shadow-md border border-gray-200">
                            ...
                            <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-500 pl-12">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span>{{ \Carbon\Carbon::parse($agenda->tanggal_agenda)->translatedFormat('l, d M Y') }}</span>
                                </div>
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span>{{ $agenda->waktu_mulai ? \Carbon\Carbon::parse($agenda->waktu_mulai)->format('H:i') : 'TBA' }} - {{ $agenda->waktu_selesai ? \Carbon\Carbon::parse($agenda->waktu_selesai)->format('H:i') : 'Selesai' }}</span>
                                </div>
                                @if ($agenda->lokasi_alamat)
                                <div class="flex items-center space-x-2">
                                    <svg class="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                    <span>{{ $agenda->lokasi_alamat }}</span>
                                </div>
                                @endif
                                @if ($agenda->ruang)
                                <div class="flex items-center space-x-2">
